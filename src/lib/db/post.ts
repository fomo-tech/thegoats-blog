...
export async function getPostBySlugTagPaginate({
  page = 1,
  pageSize = 10,
  slug,
}: {
  ...
  // 1. Lấy total count trước
  const { count, error: countError } = await supabase
    .from("posts")
    .select("id, post_tags!inner(tag_id, tags!inner(slug))", {
      count: "exact",
      head: true,
    })
  ...
  const { data, error } = await supabase
    ...
    *,
    ...
    .eq("post_tags.tags.slug", slug) // ✅ filter qua quan hệ
    .order("created_at", { ascending: false })
...
export async function getPostsPaginated(
  ...
  const [{ data: posts, error }, { count, error: countError }] =
    await Promise.all([
      supabase
        .from("posts")
        .select("*", { count: "exact" })
...
export async function getPostById(id: string): Promise<Post | null> {
  ...
  if (error) throw new Error("Lỗi khi lấy bài viết: " + error.message);
  const tags =
    data.post_tags?.map((pt: { tag: { id: string } }) => pt.tag.id) || [];
...
export async function updatePost(
  ...
  if (deleteError)
    throw new Error("Lỗi khi xoá tag cũ: " + deleteError.message);
