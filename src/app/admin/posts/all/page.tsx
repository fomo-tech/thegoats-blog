...
import { deletePost, getPostsPaginatedWithFilters, Post } from '@/lib/db/post'
...
const PostLists = () => {
    ...
    useEffect(() => {
        const loadMeta = async () => {
            const [cats, tagList] = await Promise.all([getAllCategories(), getAllTags()])
            setCategories(cats)
            setTags(tagList)
        }
        loadMeta()
    ...
    const columns = [
        ...
        },
        {
            ...
            render: (published: boolean) => (
                <span className={`text-sm font-medium ${published ? 'text-green-600' : 'text-gray-400'}`}>
        ...
        },
        {
            ...
            render: (_: any, record: any) => (
                <div className="flex gap-2">
                    <button className="bg-transparent" onClick={() => router.push(adminRoutes('post.id', record?.id))}>
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="text-cyan-500"
                            style={{ width: '20px' }}>
                    ...
                    <button className="bg-transparent" onClick={() => handleDelete(record.id)}>
                        <svg
                            xmlns="http://www.w3.org/2000/svg"
                            fill="none"
                            viewBox="0 0 24 24"
        ...
        <PageContent>
            <div className="space-y-6">
                ...
                <div className="flex justify-between items-center mt-4">
                    <span className="text-sm text-gray-600">Tổng cộng: {total} bài viết</span>
